...
class TestSafeToPlay(unittest.TestCase):
    def setUp(self):
        ...
        self.trump = "Spades"
        self.current_hand = MagicMock()
        self.current_hand.current_trick = MagicMock()
        self.current_hand.trump = self.trump
        self.current_hand.current_trick.lead_suit = "Trump"
        ...
        self.current_hand.current_trick.cards = self.cards
    ...
    def test_jick_is_safe_after_a_k_q_j(self):
        ...
        self.current_hand.current_trick.cards = [ self.two_diamonds ]
        safe = self.cc.safe_to_play(0, self.jack_clubs, self.current_hand)
        self.assertEqual(safe, True)


    def test_two_is_safe_after_everyone_is_out_of_trump(self):
        self.cc.card_was_played(0, self.ace_spades, self.current_hand)
        self.cc.card_was_played(1, self.two_diamonds, self.current_hand)
        self.cc.card_was_played(2, self.queen_hearts, self.current_hand)
        self.cc.card_was_played(3, self.seven_clubs, self.current_hand)

        self.current_hand.current_trick.cards = []
        safe = self.cc.safe_to_play(0, self.two_spades, self.current_hand)
        self.assertEqual(safe, True)

    def test_jick_is_safe_if_only_player_behind_me_is_out(self):
        self.cc.player_out_of_cards[1]["Trump"] = True
        self.current_hand.current_trick.cards = [ self.two_diamonds, self.seven_clubs ]
        safe = self.cc.safe_to_play(0, self.jack_clubs, self.current_hand)
        self.assertEqual(safe, True)
...
class TestHighestCardStillOut(unittest.TestCase):
    def setUp(self):
        ...
        self.current_hand.current_trick = MagicMock()
        self.current_hand.trump = self.trump
        self.current_hand.current_trick.lead_suit = self.trump
